...
--
-- Host: localhost:3306
-- Generation Time: Jun 10, 2023 at 01:08 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkout_read` ()   BEGIN
 SELECT checkout.id_checkout,transactions.riotId,  	virtual_account.name, checkout.total_payment,  virtual_account.number, checkout.timestamp, checkout.status FROM checkout
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_create` (IN `id_user` INT(25) UNSIGNED ZEROFILL, IN `role_id` INT(25), IN `email` VARCHAR(30), IN `username` VARCHAR(30), IN `password` VARCHAR(50), IN `name` VARCHAR(30))   BEGIN
...
--
ALTER TABLE `virtual_account`
  MODIFY `id_virtual_account` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
